...
class ColumnMappingDialog(tk.Toplevel):
    def __init__(self, parent: tk.Misc, headers: list[str]):
        ...
        ttk.Label(
            self,
            text="Choose the two columns to import. Other CSV columns will be ignored.",
        ...
        buttons.grid(row=3, column=0, columnspan=2, padx=16, pady=16, sticky="e")
    ...
    def confirm(self) -> None:
        ...
        if id_index < 0 or name_index < 0:
            messagebox.showerror("Columns missing", "Choose both columns.", parent=self)
            return
        if id_index == name_index:
            messagebox.showerror("Columns duplicated", "Product ID and name must use different columns.", parent=self)
            return
        self.result = (id_index, name_index)
        self.destroy()
