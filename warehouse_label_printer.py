...
from print_layout.warehouse_label_pdf import Cell, Product, label_count, render_labels_pdf
...
class LabelPrinterApp:
    ...
    def _build_ui(self):
        ...
        ttk.Label(drawing_panel, text="Blue = selected for printing    Green = occupied    Gray = empty").pack(anchor="w", pady=(7, 0))
    ...
    def draw_shelf(self):
        ...
        for screen_row, (number, count) in enumerate(reversed(shelf.rows)):
            ...
            for cell in by_row[number]:
                ...
                fill = "#cce4ff" if marked else "#e4efdf" if cell.products else "#f6f6f6"
    ...
    def update_summary(self):
        selected = self.selected_shelf_ids()
        self.summary_text.set(f"{len(selected)} shelf(s) selected / {len(self.marked)} cells marked. Print order: ground row upward, left to right.")
